...
/*   Updated: 2022/01/18 20:02:32 by guhernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//
		class Folder {
			...
			public:
				...
				virtual Folder		&_init_folder() {
					...
					for (iterator_files_names it = this->_files_names.begin() ;
						...
						for (int end_id = 0 ; test_file.is_open() ; ++end_id) {
							...
							test_file.open(this->_turn_in_directory + _end_id + "/" + test_filename);
						}
						it->second = test_filename;
						this->_files[it->second]= new std::ofstream(this->_turn_in_directory + _end_id + it->second);
					}
					return *this;
				}
				...
				std::ofstream			&get_stream(const std::string &key) {
					return *_files[key];
				}
...
//
	protected:
		...
		ITester(const ITester &source)
			: _test_nb(source._test_nb),
			_exec_time(source._exec_time), _folder_std(NULL), _folder_ft(NULL) {
		}
